...
namespace Core {
	...
	real32 Vec2::angle(const Vec2 &vectorA, const Vec2 &vectorB) {
		// Here use a small trick to avoid the situation when lose precision in calculating a real32 number.
	...
	real32 Vec2::getX(const Vec2 &vector) {
		return vector.mX;
	}

	real32 Vec2::getY(const Vec2 &vector) {
		return vector.mY;
	}

	void Vec2::set(Vec2 &vector, real32(&array)[2]) {
		vector = Vec2(array);
	}

	void Vec2::setX(Vec2 &vector, real32 x) {
		vector.mX = x;
	}

	void Vec2::setY(Vec2 &vector, real32 y) {
		vector.mY = y;
	}
