...
Mat4::Mat4(real32 m11, real32 m12, real32 m13, real32 m14, real32 m21,
           real32 m22, real32 m23, real32 m24, real32 m31, real32 m32,
           real32 m33, real32 m34, real32 m41, real32 m42, real32 m43,
           real32 m44)
    : mMatrix{m11, m12, m13, m14, m21, m22, m23, m24,
              m31, m32, m33, m34, m41, m42, m43, m44} {}

Mat4::Mat4(real32 (&array)[16])
    : mMatrix{array[0],  array[1],  array[2],  array[3], array[4],  array[5],
              array[6],  array[7],  array[8],  array[9], array[10], array[11],
              array[12], array[13], array[14], array[15]} {}

Mat4::Mat4(const Mat4 &other)
    : mMatrix{other.mMatrix[0],  other.mMatrix[1],  other.mMatrix[2],
              other.mMatrix[3],  other.mMatrix[4],  other.mMatrix[5],
              other.mMatrix[6],  other.mMatrix[7],  other.mMatrix[8],
              other.mMatrix[9],  other.mMatrix[10], other.mMatrix[11],
              other.mMatrix[12], other.mMatrix[13], other.mMatrix[14],
              other.mMatrix[15]} {}
...
Vec4 Mat4::mul(const Mat4 &matrix, const Vec4 &vector) {
  ...
  RF_ALIGN16 real32 vectorA[4] = {vector.getX(), vector.getY(), vector.getZ(),
                                  vector.getW()};
  __m128 sseA;
  sseA = _mm_load_ps(vectorA);
  for (int i = 0; i <= 12; i += 12) {
    ...
    sseResult = _mm_mul_ps(sseA, sseB);
    _mm_store_ps(vectorResult, sseResult);
    tempArray[static_cast<int>(i >> 2)] =
        vectorResult[0] + vectorResult[1] + vectorResult[2] + vectorResult[3];
  }
...
Mat4 Mat4::mul(const Mat4 &matrixA, const Mat4 &matrixB) {
  real32 tempArray[16] = {0.0f};
  for (int i = 0; i < 16; ++i) {
    RF_ALIGN16 real32 vectorA[4] = {
        matrixA.mMatrix[static_cast<int>(i >> 2)],
        matrixA.mMatrix[static_cast<int>(i >> 2) + 1],
        matrixA.mMatrix[static_cast<int>(i >> 2) + 2],
        matrixA.mMatrix[static_cast<int>(i >> 2) + 3]};
    RF_ALIGN16 real32 vectorB[4] = {
        matrixB.mMatrix[(i % 4)], matrixB.mMatrix[(i % 4) + 4],
...
real32 Mat4::determinant(const Mat4 &matrix) {
  RF_ALIGN16 real32 vectorA[4] = {matrix.mMatrix[0], matrix.mMatrix[0],
                                  matrix.mMatrix[0], matrix.mMatrix[1]};
  RF_ALIGN16 real32 vectorB[4] = {matrix.mMatrix[5], matrix.mMatrix[6],
                                  matrix.mMatrix[7], matrix.mMatrix[6]};
  RF_ALIGN16 real32 vectorC[4] = {matrix.mMatrix[1], matrix.mMatrix[2],
                                  matrix.mMatrix[8], matrix.mMatrix[8]};
  RF_ALIGN16 real32 vectorD[4] = {matrix.mMatrix[7], matrix.mMatrix[7],
                                  matrix.mMatrix[13], matrix.mMatrix[14]};
  RF_ALIGN16 real32 vectorE[4] = {matrix.mMatrix[8], matrix.mMatrix[9],
                                  matrix.mMatrix[9], matrix.mMatrix[10]};
  RF_ALIGN16 real32 vectorF[4] = {matrix.mMatrix[15], matrix.mMatrix[14],
                                  matrix.mMatrix[15], matrix.mMatrix[15]};
  RF_ALIGN16 real32 vectorG[4] = {matrix.mMatrix[1], matrix.mMatrix[2],
                                  matrix.mMatrix[3], matrix.mMatrix[2]};
  RF_ALIGN16 real32 vectorH[4] = {matrix.mMatrix[4], matrix.mMatrix[4],
                                  matrix.mMatrix[4], matrix.mMatrix[5]};
  RF_ALIGN16 real32 vectorI[4] = {matrix.mMatrix[3], matrix.mMatrix[3],
                                  matrix.mMatrix[9], matrix.mMatrix[10]};
  RF_ALIGN16 real32 vectorJ[4] = {matrix.mMatrix[5], matrix.mMatrix[6],
                                  matrix.mMatrix[12], matrix.mMatrix[12]};
  RF_ALIGN16 real32 vectorK[4] = {matrix.mMatrix[11], matrix.mMatrix[10],
                                  matrix.mMatrix[11], matrix.mMatrix[11]};
  RF_ALIGN16 real32 vectorL[4] = {matrix.mMatrix[12], matrix.mMatrix[13],
                                  matrix.mMatrix[13], matrix.mMatrix[14]};
  ...
  __m128 sseA, sseB, sseC, sseD, sseE, sseF, sseG, sseH, sseI, sseJ, sseK, sseL,
      sseM, sseN, sseO, sseP, sseResultA, sseResultB, sseResultC;
  sseA = _mm_load_ps(vectorA);
  sseB = _mm_load_ps(vectorB);
  ...
  RF_ALIGN16 real32 vectorM[4] = {vectorResultA[0], vectorResultA[1],
                                  vectorResultA[2], 0.0f};
  RF_ALIGN16 real32 vectorN[4] = {vectorResultC[3], vectorResultC[2],
                                  vectorResultC[1], 0.0f};
  RF_ALIGN16 real32 vectorO[4] = {vectorResultA[4], vectorResultB[0],
                                  vectorResultB[1], 0.0f};
  RF_ALIGN16 real32 vectorP[4] = {vectorResultC[0], vectorResultB[3],
                                  vectorResultB[2], 0.0f};
  ...
  return vectorResultD[0] - vectorResultD[1] + vectorResultD[2] +
         vectorResultE[0] - vectorResultE[1] + vectorResultE[2];
}
...
Vec4 Mat4::getRow(const Mat4 &matrix, int row) {
  assert(row > 0 && row < 5);
  return Vec4(matrix.mMatrix[(row - 1) << 2],
              matrix.mMatrix[((row - 1) << 2) + 1],
              matrix.mMatrix[((row - 1) << 2) + 2],
              matrix.mMatrix[((row - 1) << 2) + 3]);
}

Vec4 Mat4::getCol(const Mat4 &matrix, int col) {
  assert(col > 0 && col < 5);
  return Vec4(matrix.mMatrix[col - 1], matrix.mMatrix[col - 1 + 4],
              matrix.mMatrix[col - 1 + 8], matrix.mMatrix[col - 1 + 12]);
}
