...
Mat3::Mat3(real32 m11, real32 m12, real32 m13, real32 m21, real32 m22,
           real32 m23, real32 m31, real32 m32, real32 m33)
    : mMatrix{m11, m12, m13, m21, m22, m23, m31, m32, m33} {}

Mat3::Mat3(real32 (&array)[9])
    : mMatrix{array[0], array[1], array[2], array[3], array[4],
...
Mat3 Mat3::mul(const Mat3 &matrixA, const Mat3 &matrixB) {
  ...
  for (int i = 0; i < 9; ++i) {
    RF_ALIGN16 real32 vectorA[4] = {
        matrixA.mMatrix[static_cast<int>(i / 3)],
        matrixA.mMatrix[static_cast<int>(i / 3) + 1],
        matrixA.mMatrix[static_cast<int>(i / 3) + 2], 0.0f};
    RF_ALIGN16 real32 vectorB[4] = {matrixB.mMatrix[(i % 3)],
                                    matrixB.mMatrix[(i % 3) + 3],
...
real32 Mat3::determinant(const Mat3 &matrix) {
  RF_ALIGN16 real32 vectorA[4] = {matrix.mMatrix[0], matrix.mMatrix[1],
                                  matrix.mMatrix[2], 0.0f};
  RF_ALIGN16 real32 vectorB[4] = {matrix.mMatrix[4], matrix.mMatrix[5],
                                  matrix.mMatrix[3], 0.0f};
  RF_ALIGN16 real32 vectorC[4] = {matrix.mMatrix[8], matrix.mMatrix[6],
                                  matrix.mMatrix[7], 0.0f};
  RF_ALIGN16 real32 vectorD[4] = {matrix.mMatrix[2], matrix.mMatrix[0],
                                  matrix.mMatrix[1], 0.0f};
  RF_ALIGN16 real32 vectorE[4] = {matrix.mMatrix[4], matrix.mMatrix[5],
                                  matrix.mMatrix[3], 0.0f};
  RF_ALIGN16 real32 vectorF[4] = {matrix.mMatrix[6], matrix.mMatrix[7],
                                  matrix.mMatrix[8], 0.0f};
  ...
  return vectorResultA[0] + vectorResultA[1] + vectorResultA[2] -
         vectorResultB[0] - vectorResultB[1] - vectorResultB[2];
}
...
Vec3 Mat3::getRow(const Mat3 &matrix, int row) {
  assert(row > 0 && row < 4);
  return Vec3(matrix.mMatrix[(row - 1) * 3],
              matrix.mMatrix[((row - 1) * 3) + 1],
              matrix.mMatrix[((row - 1) * 3) + 2]);
}

Vec3 Mat3::getCol(const Mat3 &matrix, int col) {
  assert(col > 0 && col < 4);
  return Vec3(matrix.mMatrix[col - 1], matrix.mMatrix[col - 1 + 3],
              matrix.mMatrix[col - 1 + 6]);
}
