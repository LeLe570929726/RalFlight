...
public:
  ...
  template <class T> static T sqrt(T scalar) {
    return static_cast<T>(std::sqrt(static_cast<double>(scalar)));
  }
  template <class T> static T ln(T scalar) {
