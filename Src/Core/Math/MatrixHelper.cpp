...
Mat4 MatrixHelper::createZero() { return Mat4::zero; }

Mat4 MatrixHelper::createPerspective(real32 fov, real32 near, real32 far,
                                     real32 ratio) {
  assert(near != far);
  assert(ratio);
  assert(fov);
...
Mat4 MatrixHelper::createOrtho(real32 width, real32 height, real32 near,
                               real32 far) {
  assert(width);
  assert(height);
  assert(near != far);
